...
- Надо представлять последствия потери AZ. Будет рост запросов api server, нагрузка etcd, будет происходить решедулинг, должен сработать circuit breaker, нужно убедиться что в проблемную AZ не будет идти остаточного трафика, например какие нибудь Sticky sessions, кеши.
